...
def gerenciar_evento(request):
    if request.method == "GET":
        nome2 = request.GET.get('nome')
        eventos = Evento.objects.filter(criador = request.user) 
        print(eventos)
        if nome2:
            eventos = eventos.filter(nome__contains= nome2)
            
        #aqui está filtrando um nome específico de evento dentre 
        # todos os eventos desse usuário que contenha ao menos 
        # uma parte da string, assi  não precisfrom django.http import Http404a lembrar o nome todo
        #print(eventos )
        return render(request, 'gerenciar_evento.html',{'eventos':eventos})

#@inscrever_evento 
def inscrever_evento(request,id): 
    #return HttpResponse('teste inscrever eventos')
    evento = get_object_or_404(Evento, id=id)
    if request.method == "GET":
        return render(request,'inscrever_evento.html',{'evento': evento})
    
    elif request.method == "POST":
        #validar se usuário já está cadastrado no evento
...
def participantes_evento(request, id):
    #isso é reinderizar a página
    evento = get_object_or_404(Evento, id=id)
    if not evento.criador == request.user:
        raise Http404(' você não é proprietário desse evento')
    if request.method == 'GET':
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        fonte_nome = ImageFont.truetype(path_fonte,180)
        fonte_info = ImageFont.truetype(path_fonte,90)

        draw.text((230, 651),f"{participante.username}", fonte = fonte_nome, fill = (0,0,0))
        draw.text((761, 782),f"{evento.nome}", fonte = fonte_info, fill = (0,0,0))
        draw.text((816, 849),f"{evento.carga_horaria} horas", fonte = fonte_info, fill = (0,0,0))
...
def procurar_certificado(request, id):
    ...
    certificado = Certificado.objects.filter(evento =  evento,participante__email=email).first()
